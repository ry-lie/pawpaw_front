    ...
    useEffect(() => {
        const loadRoomList = async () => {
            try {
                ...
                if (currentUser) {
                    const userRooms = filterList.filter(room => room.participants.includes(currentUser));
                        ...
                        <li key={room.id} className="flex justify-between items-center border p-2 rounded-lg">
                            <Link href={`/chat/${room.id}?sender=${currentUser}&receiver=${otherParticipant}`}>
                                <div>
                                    ...
                                    <p className="text-gray-500 text-sm">{room.lastMessage.text}</p>
                                </div>
                            </Link>
                            <div className="flex flex-col justify-center items-center">
                                <Button btnType="submit" onClick={() => deleteChatRoom(room.id)} containerStyles="!text-base !text-black bg-transparent hover:underline hover:bg-transparent">
                                    나가기
                                </Button>
                                {newMessage[room.id] && (
                                    <Image src={NewMessage} alt="new_message" className="h-6 w-6 " />
                                )}
                            </div>
                        </li>
                    );
