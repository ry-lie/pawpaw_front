...
export default function Nav() {
  ...
  const handleRequestChat = (sender: string) => {
    const message = `${sender}님이 채팅 요청을 보냈습니다.`
    ...
    const toastId = toast(
      <div>
        ...
        <div>
          ...
          <Button onClick={() => handleReject(sender, toastId)}
  ...
  const handleApprove = (sender: string, toastId: React.ReactText) => {
    ...
    toast.info(`${sender}님의 채팅을 수락했습니다.`, {
      autoClose: 800,
      position: "top-right"
    });
  ...
  const handleReject = (sender: string, toastId: React.ReactText) => {
    toast.dismiss(toastId)
    toast.info(`${sender}님의 채팅을 거절했습니다.`, {
      autoClose: 800,
      position: "top-right"
    });
    removeAlarm(sender);
  ...
  const ApproveChatNewMessage = (alarm: AlarmProps) => {
    if (alarm.status === "accepted") {
      toast(
        <div onClick={() => moveChatRoom(alarm.sender)} className="cursor-pointer">
          <p>{alarm.sender}</p>
          <p>{alarm.message}</p>
        </div>,
        {
