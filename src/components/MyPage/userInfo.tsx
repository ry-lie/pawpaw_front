...
export default function UserInfo({ userInfo }: UserInfoProps) {
  ...
  return isMobile ? <MobileUserInfo userInfo={userInfo} /> : <DesktopUserInfo userInfo={userInfo} />;
};

const DesktopUserInfo = ({ userInfo }: UserInfoProps) => {
  return (
    <section className="w-full max-w-mobile h-36 bg-white border border-stroke_gray rounded-lg p-4 flex gap-4 items-center mb-3">
      {/* 유저 프로필 이미지 */}
      <Image src={userInfo?.profileImage || BasicProfile} alt="profile" className="w-24 h-24 bg-white rounded-full ml-3 mr-1" />

      {/* 유저 정보 */}
      <div className="flex flex-col flex-grow justify-between">
        <div className="flex items-center justify-start">
          <h3 className="text-xl font-bold">{userInfo?.name || "닉네임이 없습니다"}</h3>
        </div>
...
const MobileUserInfo = ({ userInfo }: UserInfoProps) => {
  return (
    <section className="w-full h-auto bg-white border border-stroke_gray rounded-lg px-3 pt-3 py-2 flex flex-col gap-2 items-start mb-3">
      <div className="flex">
        {/* 유저 프로필 이미지 */}
        <Image src={userInfo?.profileImage || BasicProfile} alt="profile" className="w-16 h-16 bg-white rounded-full mr-4" />

        {/* 유저 정보 */}
        <div className="flex flex-col flex-grow justify-center">

          <div className="flex items-center justify-start">
            <h3 className="text-lg font-bold mr-2">{userInfo?.name || "견주"}</h3>
            <Link href={PATHS.MY_INFO_MODIFY}>
